...
export const useScheduleRequests = (
  filters: { period_id?: string; store_id?: string; date_from?: string; date_to?: string; page?: number; per_page?: number } = {},
...
export const useUpdateRequestStatus = (): UseMutationResult<ScheduleRequestItem, Error, { id: string; status: string }> => {
  const qc = useQueryClient();
  return useMutation<ScheduleRequestItem, Error, { id: string; status: string }>({
    mutationFn: async ({ id, status }) => {
      const res: AxiosResponse<ScheduleRequestItem> = await api.patch(`/admin/schedule-requests/${id}/status`, { status });
      return res.data;
    },
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["schedule-requests"] }); },
  });
