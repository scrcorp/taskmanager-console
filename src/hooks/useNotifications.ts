 ...
 *
 * Custom hook to fetch paginated notifications.
 *
 * @param page - 페이지 번호 (Page number, 1-based)
 * @param perPage - 페이지당 항목 수 (Items per page)
 * @returns 알림 목록 쿼리 결과 (Paginated notification list query result)
 */
 ...
 *
 * Custom hook to fetch the count of unread notifications.
 * Automatically refetches every 30 seconds.
 *
 * @returns 읽지 않은 알림 수 쿼리 결과 (Unread notification count query result)
 */
 ...
 *
 * Mutation hook to mark a specific notification as read.
 *
