...
/**
 * Work Roles 관리 페이지 — 매장별 Shift×Position 조합 설정.
 *
  ...
  return (
    ...
    >
      ...
      {clName && checklistId ? (
        ...
        <button
          onClick={() => router.push(`/stores/${storeId}?tab=checklists`)}
...
export default function WorkRolesPage(): React.ReactElement {
  ...
  const openEdit = (role: WorkRole) => {
    ...
    setHeadcount(String(role.required_headcount));
  ...
  const handleSave = async () => {
    ...
    try {
      const data: WorkRoleUpdate = {
        ...
        required_headcount: parseInt(headcount) || 1,
